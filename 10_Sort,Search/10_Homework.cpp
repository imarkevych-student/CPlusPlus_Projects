...
template <typename T>
void ShowArray(T arr[], int size, string sortType)
{
	for (int i = 0; i < size; i++)
	{
		cout << arr[i] << " ";
...
template <typename T>
void QuickSort(T arr[], int B, int E)
{
...
int main()
{
	...
	int arr[size];
	InitArray(arr, size);
	ShowArray(arr, size, "unsorted");
	SelectSort(arr, size);
	ShowArray(arr, size, "Select Sort");
	BubbleSort(arr, size);
	ShowArray(arr, size, "Bubble sort");
	ShakerSort(arr, size);
	ShowArray(arr, size, " Shaker Sort");
	QuickSort(arr, 0, size - 1);
	ShowArray(arr, size, "Quick sort");
